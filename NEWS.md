...
required for large matrices in `proj\_LogBip()`.
...
* `sdv\_MM()`: loop variable `j` initialised before the loop to prevent a
...
Version 1.1.1 introduced a major new fitting method for the logistic biplot
model, described in:

> Babativa-Marquez, J. G., \& Vicente-Villardon, J. L. (2026).
...
**theta**\_i = mu\_i + sum\_s a\_is \* b\_s. Consequently:
...
and **mu**, the row markers of any new observation x\_new are simply:
a\_new = (x\_new - mu') \* V. No optimisation is required.
3. **Missing data are handled natively.** A weight matrix **W** (W\_ij = 1 if
x\_ij is observed, 0 if missing) is incorporated into the loss function.
...
(using `RSpectra::eigs\_sym()` for large matrices).
...
* The returned object includes an `impute\_x` component: the completed binary
...
imputed\_data <- res\_PDLB$impute\_x   # completed matrix
```
...
```r
out <- proj\_LogBip(x = x\_miss, k = 2, max\_iters = 1000, epsilon = 1e-5)
# out$mu      -- estimated intercept vector (length p)
# out$A       -- row-marker matrix (n x k)
# out$B       -- column-marker matrix (p x k)
# out$x\_est   -- imputed binary matrix
# out$iter    -- number of iterations
# out$loss\_funct -- loss function values per iteration
```
...
```r
cv\_result <- cv\_LogBip(data = x\_miss, k = 0:5, method = "PDLB", maxit = 1000)
```
...
* `sdv\_MM()` -- fit a logistic biplot via the coordinate descent
...
* `pred\_LB()` -- predict binary responses and compute per-variable optimal
...
* `cv\_LogBip()` -- k-fold cross-validation for selecting the number of
dimensions.
